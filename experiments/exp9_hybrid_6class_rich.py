...
def extract_rich_features(X):
    ...
    for window in X:
        ...
        for ch in range(window.shape[1]):
            s = window[:, ch]
            row.append(np.mean(np.abs(s)))
            row.append(np.sqrt(np.mean(s**2)))
            row.append(np.var(s))
            row.append(np.sum(np.abs(np.diff(s))))
            row.append(np.sum(np.diff(np.sign(s)) != 0))
        features.append(row)
...
def main():
    print("=== Experiment 9: Hybrid SNN + rich handcrafted features (6-class) ===")
    ensure_results_dir()

    split = load_subject_split("full_6")
    train_cache_path = os.path.join(RESULTS_DIR, "exp9_hybrid_6class_rich_train_features.npz")
    test_cache_path = os.path.join(RESULTS_DIR, "exp9_hybrid_6class_rich_test_features.npz")
    print(
    ...
    print("Extracting train rich handcrafted features...")
    X_train_rich = extract_rich_features(split["X_train"])
    ...
    print("Extracting test rich handcrafted features...")
    X_test_rich = extract_rich_features(split["X_test"])

    X_train_combined = np.hstack([X_train_hidden, X_train_rich])
    X_test_combined = np.hstack([X_test_hidden, X_test_rich])
    ...
    json_path = os.path.join(RESULTS_DIR, "exp9_hybrid_6class_rich.json")
    cm_path = os.path.join(RESULTS_DIR, "exp9_hybrid_6class_rich_cm.png")
    ...
    save_json(
        json_path,
        {
            "experiment": "exp9_hybrid_6class_rich",
            "label_mode": "full_6",
            "feature_mode": "hybrid_rich",
    ...
    print("Saved results to results/exp9_hybrid_6class_rich.json")
