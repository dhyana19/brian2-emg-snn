...
def train_and_save_best_weights(X_train, y_train, weights_path):
    num_classes = len(get_class_names("rest_vs_active"))
...
def evaluate_saved_weights(X_test, y_test, weights_path):
    class_names = get_class_names("rest_vs_active")
    ...
    total_spikes_per_output = (
        np.sum(np.stack(flat_spike_counts), axis=0)
...
def main():
    print("=== Experiment 5: Pure Brian2 SNN (binary) ===")
    ensure_results_dir()
    split = load_subject_split("rest_vs_active")

    weights_path = os.path.join(RESULTS_DIR, "exp5_snn_binary_weights.npy")
    json_path = os.path.join(RESULTS_DIR, "exp5_snn_binary.json")
    cm_path = os.path.join(RESULTS_DIR, "exp5_snn_binary_cm.png")
    curve_path = os.path.join(RESULTS_DIR, "exp5_snn_binary_training_curve.png")
    ...
    save_json(
        json_path,
        {
            ...
            "total_spikes_per_output": metrics["total_spikes_per_output"],
    ...
    print("Saved results to results/exp5_snn_binary.json")
